...
@section('content')
    ...
    <div class="">
        <h1 class="text-2xl font-thin text-gray-900">Manage Users</h1>
        @if (session('success'))
        ...
        @endif
    <form action="{{ route('oauth.store') }}" method="post" class="mt-4">
        @csrf
        <div class="w-full mb-3">
            <div class="flex items-center">
                <label for="user_name" class="text-sm font-medium w-64 flex-none">Username <span class="text-red-600">*</span></label>
                <div class="flex-auto">
                    <input type="text" name="user_name" id="user_name" class="p-1 rounded-md w-full border- border-gray-400" value="" autocomplete="off">
                    <span class="text-sm text-red-600 font-normal">{{ $errors->first('user_name')}}</span>
                </div>
            </div>
        </div>
        <div class="w-full mb-3">
            <div class="flex items-center">
                <label for="password" class="text-sm font-medium w-64 flex-none">Password <span class="text-red-600">*</span></label>
                <div class="flex-auto">
                    <input type="password" name="password" id="password" class="p-1 rounded-md w-full border border-gray-400" value="">
                    <span class="text-sm text-red-600 font-normal">{{ $errors->first('password')}}</span>
                </div>
            </div>
        </div>
        <div class="w-full mb-3">
            <div class="flex items-center">
                <label for="retype_password" class="text-sm font-medium w-64 flex-none">Retype Password <span class="text-red-600">*</span></label>
                <div class="flex-auto">
                    <input type="password" name="retype_password" id="retype_password" class="p-1 rounded-md w-full border border-gray-400" value="">
                    <span class="text-sm text-red-600 font-normal">{{ $errors->first('retype_password')}}</span>
                </div>
            </div>
        </div>
    ...
    <div class="w-full overflow-scroll h-64 bg-gray-100 mt-5 mb-4">
        <table class="w-full border border-gray-300">
            ...
            <tbody class="text-sm">
                @foreach ($user as $usr)
                    <tr>
                        <td class="border border-gray-300 text-center px-2 py-2">{{ $usr->id }}</td>
                        <td class="border border-gray-300">{{ $usr->user_name }}</td>
                        <td class="border border-gray-300">{{ $usr->password }}</td>
                        <td class="border border-gray-300 text-center">
                            <a href="javascript:void(0)" class="edit-button cursor-pointer px-1 py-1 text-blue-800" title="Edit" id="editButton" data-url="{{ route('oauth.edit', $usr->id) }}"><i class="bi bi-pencil-square"></i>&nbsp;Edit</a>
                            <a type="button" class="remove cursor-pointer px-1 py-1 text-blue-800" id="{{ $usr->id }}" title="Delete"><i class="bi bi-trash"></i>&nbsp;Delete</a>
                        </td>
                    </tr>
                @endforeach
            </tbody>
    ...
    <div class="fixed w-full top-10 h-full left-0 close-dialog bg-gray-500/50 ml-28" id="confirmDialog">
        <div class="shadow-md bg-white w-96 m-auto mt-36 h-28 rounded-md p-3">
            <p class="text-sm text-gray-500 mt-3">Are you sure to delete this record? This action can't be undo</p>
                ...
                <form action="" method="post">
                    <div class="px-6 py-6 space-y-6">
                        ...
                        <div class="w-full mb-3">
                            ...
                            <input type="text" name="user_name" id="user_name_2" class="p-1 rounded-md w-full border border-gray-400" value="">
                        </div>
                    </div>
                    <!-- Modal footer -->
    ...
    <script src="{{ asset('js/edit-users.js') }}" type="text/javascript"></script>
@endsection
